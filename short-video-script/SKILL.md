...
1. **输出脚本到对话**：先输出符合标准格式的完整脚本，供用户在对话中查看

2. **写入文件保存**：将脚本写入项目目录下的 `scripts/` 目录（如 `d:\workspace\my-awesome-skills\scripts\`），文件名格式为：`YYYY-MM-DD-视频标题.md`
   - 确保目录存在（不存在则创建）
   - 使用规范的文件名格式（标题中的空格替换为短横线）
   - 包含完整的脚本内容，采用标准脚本输出格式
...
- **叠加层**：HTML动画（HyperFrames生成）- 在基础画面上叠加的文字、图形、动效

| 时间 | 画面 | 台词/字幕 | 音效/备注 |
|------|------|----------|----------|
| 0-3s | 基础：博主面对镜头<br>叠加：标题文字动画 | 【钩子台词】 | BGM渐入，节奏感强 |
| 3-Xs | 基础：[录屏/拍摄内容]<br>叠加：[HTML动画描述] | [台词] | [演示/操作/特效] |
| ... | ... | ... | ... |

**画面填写示例**：
- 基础：录屏展示工具界面，鼠标点击按钮
- 叠加：圆圈高亮按钮位置 + "一键生成"文字弹出动画
- 基础：摄像头博主口播
- 叠加：右侧关键信息卡片滑入
...
**叠加层动画**：
- 动画类型：文字卡片/高亮圆圈/箭头指示/图表
- 出现时机：与台词同步点
- 风格参考：极简科技风/手绘风格/渐变霓虹
